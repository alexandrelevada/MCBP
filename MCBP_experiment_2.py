...
Python script to reproduce the second set of experiments in the paper
...
#X = skdata.fetch_openml(name='AP_Omentum_Ovary', version=1)
...
sc1 = silhouette_score(dados, kmeans.labels_)
ch1 = calinski_harabasz_score(dados, kmeans.labels_)
db1 = davies_bouldin_score(dados, kmeans.labels_)
...
filtered_centers, indices = kmeans_plusplus(filtered, n_clusters=c, random_state=42)
kmeans = KMeans(n_clusters=c, init=filtered_centers).fit(dados)
sc2 = silhouette_score(dados, kmeans.labels_)
ch2 = calinski_harabasz_score(dados, kmeans.labels_)
db2 = davies_bouldin_score(dados, kmeans.labels_)
print('K-means++ on original data with centroids obtained from filtered data')
print('SC: ', sc2)
print('CH: ', ch2)
print('DB: ', db2)
print()
...
#print()
#rint(str(sc1)+'\t'+str(ch1)+'\t'+str(db1)+'\t'+str(sc2)+'\t'+str(ch2)+'\t'+str(db2))
